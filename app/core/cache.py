...
import lz4.frame
from app.shared_state import last_known_price_in_memory
...
async def set_last_known_price(redis_client: Redis, symbol: str, price_data: dict):
    """
    Store the last known price data for a symbol in in-memory storage.
    """
    symbol = symbol.upper()
    last_known_price_in_memory[symbol] = price_data.copy() if price_data else {}

async def get_last_known_price(redis_client: Redis, symbol: str) -> Optional[dict]:
    """
    Retrieve the last known price data for a symbol from in-memory storage.
    """
    symbol = symbol.upper()
    return last_known_price_in_memory.get(symbol, None)
